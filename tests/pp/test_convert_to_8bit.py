...
def test_convert_16_to_8bit(ds_image):
    ...
    ds = ds_image.pp["DAPI"].pp.add_layer(np.random.randint(0, 2**16, img_size).astype(np.uint16), "_16bit_image")
...
def test_convert_32_to_8bit(ds_image):
    img_size = (ds_image.sizes["x"], ds_image.sizes["y"])
    ds = ds_image.pp["DAPI"].pp.add_layer(np.random.randint(0, 2**32, img_size).astype(np.uint32), "_32bit_image")
...
def test_convert_float_to_8bit(ds_image):
    img_size = (ds_image.sizes["x"], ds_image.sizes["y"])
    ds = ds_image.pp["DAPI"].pp.add_layer(np.random.rand(*img_size).astype(np.float64), "_float_image")
...
def test_convert_float_with_values_larger_than_one(ds_image):
    img_size = (ds_image.sizes["x"], ds_image.sizes["y"])
    ds = ds_image.pp["DAPI"].pp.add_layer(np.random.rand(*img_size) * 2, "_float_image")
...
def test_convert_float_with_values_smaller_than_zero(ds_image):
    img_size = (ds_image.sizes["x"], ds_image.sizes["y"])
    ds = ds_image.pp["DAPI"].pp.add_layer(np.random.rand(*img_size) - 1, "_float_image")
    with pytest.raises(
