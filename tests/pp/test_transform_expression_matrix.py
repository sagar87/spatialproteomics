...
def test_invalid_mode(ds_labels):
    with pytest.raises(
        ...
        ds_labels.pp.add_quantification().pp.transform_expression_matrix(method="dummy")
...
def test_layer_not_available(ds_labels):
    with pytest.raises(
        ...
        ds_labels.pp.add_quantification().pp.transform_expression_matrix(key="dummy_layer")


def test_clip(ds_labels):
    expression_matrix = ds_labels.pp.add_quantification().pp.get_layer_as_df(Layers.INTENSITY)
    expression_matrix_transformed = (
        ds_labels.pp.add_quantification()
...
def test_double_zscore(ds_labels):
    ...
    ds_labels.pp.add_quantification().pp.transform_expression_matrix(method="double_zscore").pp.get_layer_as_df(
        Layers.INTENSITY
    )
