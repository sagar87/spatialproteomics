...
def test_predict_cell_types_argmax(ds_labels_spatialdata):
    sp.la.predict_cell_types_argmax(ds_labels_spatialdata, ct_dict, copy=True)


def test_predict_cell_types_argmax_no_quantification(ds_segmentation_spatialdata):
    # no quantification layer found
    with pytest.raises(
        AssertionError,
        match="Tables key table not found in spatial data object",
    ):
        sp.la.predict_cell_types_argmax(ds_segmentation_spatialdata, ct_dict)


def test_predict_cell_types_argmax_invalid_markers(ds_labels_spatialdata):
    ...
    with pytest.raises(AssertionError, match="The following markers were not found in quantification layer"):
        ...
        sp.la.predict_cell_types_argmax(ds_labels_spatialdata, ct_dict)


# def test_predict_cell_types_argmax_without_overwriting_existing_annotations(ds_segmentation_spatialdata):
