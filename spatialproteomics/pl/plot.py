...
class PlotAccessor:
    ...
    def histogram(self, feature: str, key: str = Layers.OBS, bins: int = 50, ax=None):
        """
        Plot a histogram of a given feature in a layer.

        Parameters:
        ----------
        feature : str
            The name of the feature to plot the histogram for.
        key : str, optional
            The key of the layer to use. Defaults to Layers.OBS.
        bins : int, optional
            The number of bins to use in the histogram. Defaults to 50.
        ax : matplotlib.axes.Axes, optional
            The axes object to plot the histogram on. If not provided, a new axes object will be created.
        ...
        ------
        AssertionError
            If the provided key is not found in the object.
        AssertionError
            If the provided feature is not found in the layer.

        """
        # checking if the key is in the object
        assert key in self._obj, f"Could not find layer with key {key}. Please provide a valid key."

        if ax is None:
            ax = plt.gca()

        df = self._obj.pp.get_layer_as_df(key, celltypes_to_str=False)

        # checking that the feature is in the dataframe
        assert feature in df.columns, f"Could not find feature {feature} in layer {key}."
